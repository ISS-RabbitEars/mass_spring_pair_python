...
xlo[:] = x[0] + np.sign((y[1] - y[0]) * flipa * flipb) * mra[0] * np.sin(theta)
ylo[:] = y[0] + mra[0] * np.cos(theta)
...
xlf[:] = x[1] - mra[1] * np.sign((y[1]-y[0])*flipa*flipb) * np.sin(theta)
ylf[:] = y[1] - mra[1] * np.cos(theta)
